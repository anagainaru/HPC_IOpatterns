...
def find_clusters(events_time):
    ...
    while len(events_delay) > 0:
        ...
        if max_delay <= 60 and best_clusters[0] == 0 and max(labels) > 0:
            ...
            print("[dbg] Store current cluster with a score of", silhouette_avg)
...
def find_patterns(labels, events_type, events_time, pattern_list, rank):
    ...
    print("[dbg] The current TAU log:",log)
    return pattern_list, log
...
def interpolate_comp_pattern(log, total_exec, req_exec, rank):
    ...
    for pattern in pattern_ids:
        ...
        try:
            ...
            fit = np.polyfit(range(len(ts_series)), ts_series, 2)
...
def from_pattern_create_log(log, input_file, output_file):
    max_exec = max(max([i[2] for i in log[rank]]) for rank in log)
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print("Usage: python %s log_file num_ranks exec_time [output_file]" %(sys.argv[0]))
        exit()
    
    req_ranks = int(sys.argv[2])
    req_exec = int(sys.argv[3])
    ...
    from_pattern_create_log(log, sys.argv[1], output_file)
